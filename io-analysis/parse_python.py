...
def parse_file(filename):
    ...
    def gen_identifier(identifier, node_type = 'identifier'):
        ...
        json_node['value'] = str(identifier)
        return pos
    ...
    def traverse(node):
        ...
        #    children.append(traverse_list(node.finalbody, 'finalbody'))
            ...
            #string? type_comment
            if node.returns:
                children.append(gen_identifier(node.returns, 'returns'))
            #if node.type_comment:
            #    children.append(gen_identifier(node.type_comment, 'type_comment'))
        else:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        PrintUsage()
    #try:
    json_tree = parse_file((sys.argv[1]))
    ...
    ast = json.loads(json_tree)
    print(ast)

    count = 0
    for token in ast:
        #if 'value' not in token and 'children' not in token:
        #    print("ERROR")
        #    print(token)
        if 'value' in token:
            if token['type'] == 'NameLoad':
               if token['value'] == 'tf':
                   print(token)
                   count += 1
